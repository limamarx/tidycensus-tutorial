...
```{r}
# get the table
tmp <- capture.output(tract_hh <- get_acs("tract",
                    table = "B11009",
...
```{r message=FALSE, warning=FALSE}
inc_df %>%
    ggplot(aes(x = Year, y = estimate, ymin = inc_lwr, ymax = inc_upr, group = variable)) +
...
raw_inc_df %>%
    ...
    mutate(moe = moe_ratio(estimate_Black, estimate_White, moe_Black, moe_White)) %>%
